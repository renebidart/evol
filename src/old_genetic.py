...
def mutate(lr_schedule):
    ...
    lr_schedule[idx] = lr_schedule[idx]*math.pow(10, random.normalvariate(0, 1))
    return lr_schedule 
...
def create_single_population(num_schedules, length=10, top=0, bottom=-6):
    """ Create Learning rate schedules, called population 
    ...
    for _ in range(0, num_schedules):
        # Create schedule
        exponents = np.random.uniform(bottom, top, epochs)
        lr_schedule = np.power(10, exponents).tolist()
        pop.append(lr_schedule)
    return pop
